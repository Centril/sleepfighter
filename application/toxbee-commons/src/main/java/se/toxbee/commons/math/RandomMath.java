...
public class RandomMath {
	 ...
	 * @return the randomly generated number.
	 */
	public static int nextRandomRanged( Random rng, final int min, final int max, boolean noZeros ) {
		return noZeros ? nextRandomRanged( rng, min, max, 0 ) : nextRandomRanged( rng, min, max );
	}
	...
	/**
	 * Like {@link #nextGaussianRanged(java.util.Random, int, int, int, double)} but excluding numbers modulo 10.
	 *
	 * @param rng the random number generator.
	 * @param min the minimum value, inclusive.
	 * @param max the maximum value, inclusive.
