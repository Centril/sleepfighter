 ...
 * @since Nov 14, 2013
 */
public abstract class MigrationExecutor extends IMigrationExecutor<MigrationUtil, Migrater> {
	private static final String TAG = MigrationExecutor.class.getSimpleName();

	abstract protected int rebuildBelowVersion();
	...
	private boolean perform( ConnectionSource cs, SQLiteDatabase db, int originVersion, int targetVersion ) {
		try {
			IMigrationException.tooOld( originVersion, this.rebuildBelowVersion() );
