...
import Tiled.TiledGameModel;


/**
 * Snake-game extension for GameModel. 
 */
public class SnakeModel extends TiledGameModel {
	/* --------------------------------
	 * Position/States.
	 * --------------------------------
	 */
	...
	/**
	 * Returns whether or not a position is the head of the snake.
	 * Since snake always has a head this method will never cause NullPointerException.
	 *
	 ...
	 * @param startDirection The direction where snakes head will be pointed at.
	...
	public SnakeModel( Dimension size, Direction startDirection, Random rng ) {
		...
		this.emptyPos = new ArrayList<Position>( size.width * size.width );
		 ...
		 *
		 * In order not to make it too difficult for client we will add snake-tiles
		 * exceeding the first one to opposite of starting-direction.
		 *
		 * Then we must add all exceeding first to the end so that the head ( index = 0 ) is first in starting direction.
		 * The starting direction must then be reset to default.
		 */
		this.snakePos = new LinkedList<Position>();
		this.addSnake( new Position( (int) Math.ceil( size.width / 2 ), (int) Math.ceil( size.height / 2 ) ) );
	...
	public void tickUpdate() throws GameOverException {
		...
		// If yes: Award client with score and add a new snake-food (if not possible -> game over).
		// If not: Transfer the previous snake tail position to empty positions and remove head from empty positions.
		...
		if ( newHeadPos.isOutOfBounds( this.getBoardSize() ) || this.snakePos.contains( newHeadPos ) ) {
			this.gameOver();
		}
	...
	private void addFood() {
		// Randomly select an empty position to remove and set current food position to that one.
		this.currFoodPos = this.emptyPos.remove( this.rng.nextInt( this.emptyPos.size() ) );
	}

	/**
	 * Helper method for adding new snake tiles to a position.
	 *
	 * @param pos The position in which to add a snake tile.
	 */
